...
function footer(doc){
  ...
  doc.text('Observações: _______________________________________________________________',14,h-33);
  doc.text('Assinatura do responsável: ______________________________________',14,h-22);
  doc.text('Relatório gerado pelo Sistema Técnico Predial',14,h-10);
}

function save(doc,name){ footer(doc); doc.save(name); }
...
export function pdfGeradores(registros=[], periodo={}){
  ...
  summaryCard(doc,14,62,55,'Horas trabalhadas',`${num(horas)} h`);
  summaryCard(doc,78,62,55,'Diesel consumido',`${num(litros)} L`,[240,253,250]);
  summaryCard(doc,142,62,55,'Custo total',brl(custo),[255,251,235]);
  autoTable(doc,{startY:96,head:[['Data','Gerador','Inicial','Final','Horas','Litros','Custo','Obs.']],body:registros.map(r=>[formatDateBR(r.data),r.gerador,r.horimetroInicial,r.horimetroFinal,num(r.horas),num(r.litros),brl(r.custo),r.observacao||'']),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]}});
...
export function pdfDieselTecnico(registros=[], periodo={}){
  ...
  doc.text('Fórmula: Consumo total mensal de diesel = consumo por hora dos 2 GMGs × tempo de funcionamento convertido em horas.',14,91,{maxWidth:180});
  autoTable(doc,{startY:108,head:[['Data','Consumo hora GMGs','Tempo','Horas decimais','Consumo total']],body:registros.map(r=>[formatDateBR(r.data),`${r.consumoHoraGMGs} L/h`,r.tempo,`${num(r.horas,4)} h`,`${num(r.total,3)} L`]),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]}});
...
export function pdfFancoil(registros=[], periodo={}){
  ...
  doc.setFontSize(10); doc.setTextColor(71,85,105);
  doc.text('Fórmulas: Consumo estimado mensal = kW × horas por dia × dias úteis. Custo estimado mensal = consumo estimado × preço kWh.',14,91,{maxWidth:180});
  autoTable(doc,{startY:108,head:[['Data','Potência kW','Horas/dia','Dias úteis','Preço kWh','Consumo','Custo']],body:registros.map(r=>[formatDateBR(r.data),r.kw,r.horasDia,r.diasUteis,`R$ ${r.precoKwh}`,`${num(r.consumo)} kWh`,brl(r.custo)]),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]}});
...
export function pdfGas(registros=[], periodo={}){
  ...
  doc.setFontSize(10); doc.setTextColor(71,85,105);
  doc.text('Fórmulas: Área comum = (m³ × fator) × 0,05. Área privativa = (m³ × fator) × 0,95.',14,91,{maxWidth:180});
  autoTable(doc,{startY:108,head:[['Data','Consumo m³','Fator','Total kg','Área comum kg','Área privativa kg']],body:registros.map(r=>[formatDateBR(r.data),r.consumoM3,r.fator,num(r.totalKg),num(r.comumKg),num(r.privativoKg)]),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]}});
  save(doc,'relatorio-gas.pdf');
}

export function pdfLocatarios(medicao, periodo={}){
  const doc=new jsPDF();
  header(doc,'Relatório de Medição de Energia dos Locatários',periodo?.mesReferencia ? {...periodo,texto:periodo.mesReferencia} : (medicao?.mes || ''));
  ...
  autoTable(doc,{startY:96,head:[['Unidade','Anterior','Atual','Consumo','Virou medidor']],body:(medicao?.linhas||[]).map(r=>[r.unidade,int(r.anterior),String(r.atual).padStart(5,'0'),int(r.consumo),r.virou?'Sim':'Não']),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]}});
  save(doc,'relatorio-locatarios.pdf');
}
...
export function pdfGeral({geradores=[],medicao,diesel=[],fancoil=[],gas=[]}, periodo={}){
  ...
  const horas=geradores.reduce((s,r)=>s+Number(r.horas||0),0);
  ...
  summaryCard(doc,14,62,42,'Horas GMGs',`${num(horas)} h`);
  ...
  summaryCard(doc,158,62,38,'Energia dos Locatários',int(medicao?.resumo?.total || 0),[239,246,255]);
  autoTable(doc,{startY:100,head:[['Indicador','Valor']],body:[['Registros de gerador',geradores.length],['Unidades medidas',medicao?.resumo?.quantidade||0],['Medidores que viraram',(medicao?.linhas||[]).filter(l=>l.virou).length],['Registros diesel mensal',diesel.length],['Registros fancoil',fancoil.length],['Gás área comum',`${num(gasComum)} kg`],['Gás área privativa',`${num(gasPriv)} kg`]],headStyles:{fillColor:[15,23,42]}});
...
function linhaCalculoEletrico(item) {
  ...
  if (tipo === 'Potência Elétrica') return [formatDateBR(item.data), tipo, `${num(r.potenciaKW)} kW`, `${num(r.aparenteKVA)} kVA`, `${num(r.reativaKVAr)} kVAr`, r.formula || ''];
  if (tipo === 'Fator de Potência') return [formatDateBR(item.data), tipo, `FP ${num(r.fp,3)}`, `${num(r.potenciaAtivaKW)} kW`, `${num(r.potenciaAparenteKVA)} kVA`, r.alerta || ''];
  if (tipo === 'Motores') return [formatDateBR(item.data), tipo, `${num(r.rpm,0)} RPM`, `${num(r.rendimento)}%`, `${r.frequencia || ''} Hz`, 'RPM e rendimento'];
  if (tipo === 'Queda de Tensão') return [formatDateBR(item.data), tipo, `${num(r.quedaV)} V`, `${num(r.quedaPercentual)}%`, r.material || '', r.alerta || ''];
  return [formatDateBR(item.data), tipo, `${num(r.valor)} ${r.unidade || ''}`, r.formula || '', '', ''];
}
...
export function pdfCalculosEletricos(calculos=[], periodo={}){
  ...
  doc.setFontSize(10); doc.setTextColor(71,85,105);
  doc.text('Este relatório reúne cálculos de apoio técnico: Lei de Ohm, potência elétrica, fator de potência, motores e queda de tensão simples.',14,91,{maxWidth:180});
  autoTable(doc,{startY:108,head:[['Data','Tipo','Resultado 1','Resultado 2','Resultado 3','Observação/Fórmula']],body:calculos.map(linhaCalculoEletrico),styles:{fontSize:8},headStyles:{fillColor:[15,23,42]},columnStyles:{5:{cellWidth:48}}});
...
export function pdfRateioAgua(registros=[], periodo={}){
  ...
  doc.setFontSize(10); doc.setTextColor(71,85,105);
  doc.text('Fórmula: Tarifa = valor total ÷ consumo total. Valores e consumos podem ser calculados automaticamente pela soma de Poço 1, Poço 2 e SABESP.',14,91,{maxWidth:180});
  autoTable(doc,{startY:108,head:[['Data','Mês','Valor Poço 1','Valor Poço 2','Valor SABESP','Valor Total','Consumo Total','Tarifa']],body:registros.map(r=>[formatDateBR(r.data),r.mesReferencia||'',brl(r.valorPoco1),brl(r.valorPoco2),brl(r.valorSabesp),brl(r.valorTotal),`${num(r.consumoTotal)} m³`,`${brl(r.tarifa)}/m³`]),styles:{fontSize:7},headStyles:{fillColor:[15,23,42]}});
