import { Copy, Droplets, FileText, Save, TrendingUp, Trash2 } from 'lucide-react';
...
function Input({ label, value, onChange, placeholder, type = 'text' }) {
  return <div>
    ...
    <input
      ...
      className="mt-2 w-full rounded-2xl border border-slate-200 p-3 outline-none focus:border-blue-400"
    />
  </div>
}
...
export default function RateioAgua(){
  ...
  const dadosAgua = [
    { name: 'Poço', value: consumoPoco },
    { name: 'SABESP', value: consumoSabesp }
  ];
  ...
  function salvar(){
    const registro = addItem('rateios_agua', {
      ...form,
      ...email,
  ...
  function salvarPdf(){
    ...
    pdfRateioAgua([registro], {
      texto: form.mesReferencia,
      dataEmissao: form.data
    });
  }

  function excluirRateio(id){
    if(!confirm('Deseja realmente excluir este rateio de água?')) return;

    deleteItem('rateios_agua', id);
    setVersao(v=>v+1);

    alert('Rateio de água excluído com sucesso.');
  }
  ...
  return <div className="space-y-6">
    <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
      <CardResumo
        titulo="Valor total"
        valor={brl(email.valorTotal)}
        icon={Droplets}
        cor="blue"
        subtitulo="Poço 1 + Poço 2 + SABESP"
      />

      <CardResumo
        titulo="Consumo total"
        valor={`${num(email.consumoTotal)} m³`}
        icon={Droplets}
        cor="teal"
        subtitulo="Consumo usado na tarifa"
      />

      <CardResumo
        titulo="Tarifa calculada"
        valor={`${brl(email.tarifa)}/m³`}
        icon={FileText}
        cor="amber"
        subtitulo="Valor total ÷ consumo total"
      />

      <CardResumo
        titulo="Registros salvos"
        valor={registros.length}
        icon={Save}
        cor="slate"
        subtitulo="Histórico de rateios"
      />
    </section>
    ...
    <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-bold text-lg text-slate-900">Poço x SABESP</h3>
        </div>

        <p className="text-sm text-slate-400 mb-4">
          Participação no consumo total do rateio atual.
        </p>

        <div className="h-56">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={dadosAgua} dataKey="value" outerRadius={78} label>
                {dadosAgua.map((_,i)=>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-2xl bg-teal-50 p-3">
            <p className="font-bold text-teal-700">Poço</p>
            <p>{num(pctPoco)}%</p>
            <p className="text-xs text-teal-700/70">{num(consumoPoco)} m³</p>
          </div>

          <div className="rounded-2xl bg-blue-50 p-3">
            <p className="font-bold text-blue-700">SABESP</p>
            <p>{num(pctSabesp)}%</p>
            <p className="text-xs text-blue-700/70">{num(consumoSabesp)} m³</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Preencha os dados mensais e gere o e-mail pronto para envio.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="mt-2 w-full rounded-2xl border border-slate-200 p-3 h-24 outline-none focus:border-blue-400"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={salvarPdf}
            className="px-5 py-3 rounded-2xl bg-slate-950 text-white font-semibold flex items-center gap-2"
          >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
        ...
        <textarea
          readOnly
          value={email.assunto}
          className="mt-2 w-full h-20 rounded-2xl border border-slate-200 p-4 text-sm bg-slate-50"
        />
        ...
        <textarea
          readOnly
          value={email.corpo}
          className="mt-2 w-full h-[520px] rounded-2xl border border-slate-200 p-4 text-sm bg-slate-50"
        />
      ...
      <Tabela
        columns={[
          ...
          {key:'tarifa',label:'Tarifa',render:r=>`${brl(r.tarifa)}/m³`},
          {
            ...
            render:r=>
              <button
                onClick={()=>pdfRateioAgua([r], { texto: r.mesReferencia, dataEmissao: r.data })}
