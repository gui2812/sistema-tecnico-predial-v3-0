...
function normalizar(txt){ return String(txt || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''); }
...
function gerarAnalise(linhas, historico) {
  return linhas.map((linha) => {
    const anteriores = historico.slice(0, 3)
      ...
      .filter(v => Number(v) > 0);
    const media3 = anteriores.length ? anteriores.reduce((s, v) => s + Number(v), 0) / anteriores.length : 0;
    const variacaoMedia = media3 ? ((Number(linha.consumo || 0) - media3) / media3) * 100 : null;
    let alerta = '';
...
export default function Locatarios(){
  ...
  const [anterior,setAnterior]=useState(anteriorDemo);
  const [atual,setAtual]=useState(atualDemo);
  const [resultado,setResultado]=useState(calcularConsumosLocatarios(anteriorDemo,atualDemo));
  ...
  const topConsumo = linhasAnalisadas.slice().sort((a,b)=>Number(b.consumo || 0)-Number(a.consumo || 0)).slice(0,5).map(l=>({ unidade:l.unidade, consumo:Number(l.consumo || 0) }));
  const chartConsumo = linhasAnalisadas
    .map(l => ({ unidade: String(l.unidade), consumo: Number(l.consumo || 0), acimaMedia: Number(l.consumo || 0) > Number(resultado.media || 0) }))
    .sort((a,b)=>String(a.unidade).localeCompare(String(b.unidade), 'pt-BR', { numeric: true }));
  const topAumento = linhasAnalisadas.filter(l => l.variacaoMedia !== null).slice().sort((a,b)=>Number(b.variacaoMedia)-Number(a.variacaoMedia)).slice(0,5);
  const alertas = linhasAnalisadas.filter(l => l.alerta);

  function calcular(){ setResultado(calcularConsumosLocatarios(anterior,atual)); }
  function salvar(){
    const r=calcularConsumosLocatarios(anterior,atual);
    const analise = gerarAnalise(r.linhas, historico);
    addItem('medicoes',{mes,dataMedicao:data,anterior,atual,linhas:r.linhas,resumo:r,analise});
    setResultado(r);
    setVersao(v=>v+1);
  ...
  return <div className="space-y-6">
    <div className="rounded-[2rem] bg-gradient-to-br from-blue-950 to-slate-950 text-white p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-2"><Zap className="text-teal-300"/><p className="font-semibold text-teal-200">Medição Mensal de Energia</p></div>
      <h1 className="text-2xl md:text-3xl font-black">Energia dos Locatários</h1>
      <p className="text-slate-300 mt-2">Cole as leituras anterior e atual, calcule o consumo em kWh, identifique medidor virado e acompanhe variações.</p>
    </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <section className="xl:col-span-2 bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
            ...
            <BarChart data={chartConsumo.length ? chartConsumo : [{ unidade: 'Sem dados', consumo: 0 }]}>
              ...
              {Number(resultado.media || 0) > 0 && (
                <ReferenceLine
                  ...
                  stroke="#0f766e"
                  strokeDasharray="6 4"
                  label={{ value: `Média ${int(resultado.media)} kWh`, position: 'insideTopRight', fill: '#0f766e', fontSize: 12 }}
                />
              )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 text-sm">
          <div className="rounded-2xl bg-teal-50 p-3"><p className="text-teal-700 font-bold">Média por unidade</p><p>{int(resultado.media)} kWh</p></div>
          <div className="rounded-2xl bg-blue-50 p-3"><p className="text-blue-700 font-bold">Total do mês</p><p>{int(resultado.total)} kWh</p></div>
          <div className="rounded-2xl bg-amber-50 p-3"><p className="text-amber-700 font-bold">Maior consumo</p><p>{resultado.maior ? `${resultado.maior.unidade} • ${int(resultado.maior.consumo)} kWh` : '-'}</p></div>
        </div>
      </section>
      <section className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
        ...
        <div className="space-y-3 max-h-72 overflow-auto pr-1">
          ...
          {alertas.slice(0,8).map((a)=><div key={a.unidade+a.alerta} className="rounded-2xl bg-amber-50 border border-amber-100 p-3 text-sm"><p className="font-bold text-amber-900">{a.unidade} • {a.alerta}</p>{a.variacaoMedia !== null && <p className="text-amber-800">Variação: {a.variacaoMedia.toFixed(0)}% vs. média 3 meses</p>}</div>)}
        </div>
      </section>
    </div>
    ...
    {topAumento.length > 0 && <section className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
      ...
      <Tabela columns={[{key:'unidade',label:'Unidade'},{key:'consumo',label:'Consumo atual',render:r=>`${int(r.consumo)} kWh`},{key:'media3',label:'Média 3 meses',render:r=>`${int(r.media3)} kWh`},{key:'variacaoMedia',label:'Variação',render:r=>`${r.variacaoMedia.toFixed(0)}%`},{key:'alerta',label:'Alerta'}]} rows={topAumento}/>
    </section>}
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="date" value={mes} onChange={e=>setMes(e.target.value)} className="mt-2 w-full rounded-2xl border p-3"/>
        </div>
        <div>
          <label className="text-sm font-semibold">Data da medição atual</label>
          <input type="date" value={data} onChange={e=>setData(e.target.value)} className="mt-2 w-full rounded-2xl border p-3"/>
        </div>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div><label className="text-sm font-semibold">Medições do mês anterior</label><textarea value={anterior} onChange={e=>setAnterior(e.target.value)} className="mt-2 w-full h-72 rounded-2xl border p-4 font-mono text-sm"/></div>
        <div><label className="text-sm font-semibold">Medições do mês atual</label><textarea value={atual} onChange={e=>setAtual(e.target.value)} className="mt-2 w-full h-72 rounded-2xl border p-4 font-mono text-sm"/></div>
      </div>
      <div className="flex flex-wrap gap-3 mt-5">
        <button onClick={calcular} className="px-5 py-3 rounded-2xl bg-blue-600 text-white font-semibold">Calcular consumo</button>
        <button onClick={salvar} className="px-5 py-3 rounded-2xl bg-teal-600 text-white font-semibold flex gap-2"><Save size={18}/>Salvar medição</button>
      </div>
    </div>
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full lg:w-auto">
          <div className="flex items-center gap-2 rounded-2xl border border-slate-200 px-3 py-2"><Search size={16} className="text-slate-400"/><input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Pesquisar unidade..." className="outline-none text-sm w-full"/></div>
          <select value={filtroAlerta} onChange={e=>setFiltroAlerta(e.target.value)} className="rounded-2xl border border-slate-200 p-3 text-sm"><option>Todos</option><option>Sem alerta</option><option>Medidor virou</option><option>Consumo zerado</option><option>Acima da média</option><option>Abaixo da média</option></select>
        </div>
      </div>
      <Tabela columns={[
        ...
        {key:'diffMedia',label:'Diferença da média',render:r=>{
          ...
          return <span className={`font-bold ${acima ? 'text-rose-600' : 'text-teal-600'}`}>{d.valor > 0 ? '+' : ''}{int(d.valor)} kWh ({d.pct > 0 ? '+' : ''}{d.pct.toFixed(0)}%)</span>;
        }},
